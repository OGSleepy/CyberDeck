...
export function CustomColumnDialog({ onAdd }: CustomColumnDialogProps) {
  ...
  // User search for autocomplete (must be after authorInput is defined)
  ...
  const handleCreate = () => {
    ...
    setIncludeReposts(false);

    toast({
      title: 'Custom column added',
      description: `${title.trim()} column has been added to your deck.`,
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        ...
        >
          <Sliders className="h-3.5 w-3.5 md:h-4 md:w-4 mr-1.5 md:mr-2" />
          Customize
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] bg-card border-border/50">
        <DialogHeader>
          <DialogTitle className="text-foreground">Create Custom Column</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            Build your own personalized feed with advanced filtering options.
          </DialogDescription>
        </DialogHeader>
